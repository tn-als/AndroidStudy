...
import com.tnals.week2.databinding.ActivityLoginBinding
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun inputData(userDataList: MutableList<User>) {
        activityResultLauncher = registerForActivityResult(
            ...
            if (it.resultCode == RESULT_OK) {
                ...
                if (user != null) {
                    binding.tvMainInputId.text = Editable.Factory.getInstance().newEditable(user.id)
                    binding.tvMainInputPw.text = Editable.Factory.getInstance().newEditable(user.pw)
    ...
    private fun goLogin(userDataList: MutableList<User>) {
        binding.tvMainLogin.setOnClickListener {
            val id = binding.tvMainInputId.text.toString()
            val pw = binding.tvMainInputPw.text.toString()
            var exist = false
